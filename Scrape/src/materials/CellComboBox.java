...
public class CellComboBox extends JComboBox<String> {
	...
	CellComboBox(JTable table, int column, Set<String> values) {
		this.table = table;
		this.column = column;
		executeColumn(values);
	}
	...
	private void addItemsFromSet(Set<String> set) {
		for (String s : set) {
			addItem(s);
		}
	}

	private void executeColumn(Set<String> values) {
		addItemsFromSet(values);
		setColumnCells();
	}
